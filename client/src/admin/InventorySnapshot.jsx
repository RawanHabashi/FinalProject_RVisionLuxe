...
export default function InventorySnapshot({ onManageClick }) {
  ...
  async function quickAdjust(item, delta) {
    try {
      ...
      // טקסט מתאים לפי פעולה
      const msg =
        delta > 0
          ? `Added 1 unit to "${item.name}"`
          : `Removed 1 unit from "${item.name}"`;
  ...
  return (
    <div className="inv-snap">
      {/* טוסט קטן מעל התיבה */}
      {toast && <div className="inv-snap-toast">{toast}</div>}
      ...
      <div className="inv-snap-kpis">
        ...
        <div className="kpi">
          ...
          <div className="kpi-value">{stats.lowCount}</div>
        </div>
        <div className="kpi">
          <div className="kpi-title">Reserved (units)</div>
          <div className="kpi-value">{stats.reservedTotal}</div>
        </div>
      </div>
